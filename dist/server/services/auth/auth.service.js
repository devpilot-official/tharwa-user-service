"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const redis_1 = __importDefault(require("@app/common/services/redis"));
const base_1 = require("@app/server/controllers/base");
const user_1 = require("@app/data/user");
const gateman_1 = __importDefault(require("@app/server/gateman"));
const DURATION_ONE_DAY = 60 * 60 * 24;
exports.DAILY_FAILED_LOGIN_TRIES = 5;
const LOGIN_TRIES_KEY = 'user:login:tries';
const LOCKED_OUT_KEY = 'user:login:locked_out';
/**
 * Handles user authentication and login rate limiting
 */
class AuthService {
    /**
     * Gets the `login tries` within a day for an user from redis
     * @param id User's id
     */
    async getLoginTries(id) {
        const key = `${LOGIN_TRIES_KEY}:${id}`;
        const loginTries = await redis_1.default.get(key);
        return Number(loginTries);
    }
    /**
     * Sets the `login tries` for an user and persists it for a day
     * @param id User's id
     * @param tries Current number of tries for the user. Defaults to `1`
     */
    async setLoginTries(id, tries = 1) {
        const key = `${LOGIN_TRIES_KEY}:${id}`;
        await redis_1.default.set(key, tries, 'EX', DURATION_ONE_DAY);
    }
    /**
     * Returns the `locked out` status for an user from redis
     * @param user User's id
     */
    async isAccountLocked(user) {
        const key = `${LOCKED_OUT_KEY}:${user}`;
        const lockedOutStatus = await redis_1.default.get(key);
        return !!lockedOutStatus;
    }
    /**
     * Sets the `locked out` status of an user to true and persists it for a day
     * @param id User's id
     */
    async lockAccount(id) {
        const key = `${LOCKED_OUT_KEY}:${id}`;
        await redis_1.default.set(key, true, 'EX', DURATION_ONE_DAY);
    }
    /**
     * Increments an user's `login tries` limit and locks the user out if they reach the allowed daily limit
     * @param id User id
     */
    async limit(id) {
        const loginTries = await this.getLoginTries(id);
        const updatedLoginTries = loginTries + 1;
        const remainingTries = exports.DAILY_FAILED_LOGIN_TRIES - updatedLoginTries;
        if (updatedLoginTries === exports.DAILY_FAILED_LOGIN_TRIES)
            await this.lockAccount(id);
        await this.setLoginTries(id, updatedLoginTries);
        throw new base_1.ControllerError(`Invalid password, you have ${remainingTries} attempt${remainingTries !== 1 ? 's' : ''} left`);
    }
    /**
     * Resets the locked out status and login limits for a user
     * @param id User's id
     */
    async reset(id) {
        if (this.getLoginTries(id))
            await redis_1.default.del(`${LOGIN_TRIES_KEY}:${id}`);
        if (this.isAccountLocked(id))
            await redis_1.default.del(`${LOCKED_OUT_KEY}:${id}`);
    }
    /**
     * Validates that an user's login credentials are valid
     * @param body Payload for a login request
     */
    async login(body) {
        const user = await user_1.UserRepo.byQuery({ email: body.email, status: 'active' }, '+password');
        const isLocked = await this.isAccountLocked(user.id);
        if (isLocked)
            throw new base_1.ControllerError("You can't login because your account has been blocked, please contact customer support");
        const isPasswordValid = await user.isPasswordValid(body.password);
        if (!isPasswordValid)
            return await this.limit(user.id);
        await this.reset(user.id);
        const token = await gateman_1.default.createSession({
            id: user.id,
            role: 'user'
        });
        return { user, token };
    }
}
exports.default = new AuthService();
//# sourceMappingURL=auth.service.js.map